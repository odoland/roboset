...
class Roboset:
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        # List of Selenium button objects. (clickable)
        self.buttons = self._makeButtonList()
        self.window_size = driver.get_window_size()  # Dictionary of width and height
        # Gets ratio of device's screen and screenshot
        self.win_ratio = self._getWindowRatio()

        # Roboset's Memory
        self.cards_list = [None] * 15  # List of SetCard objects
        # Mapping of SetCards. Key: SetCard Hash value, Value: List of indexed positions of the card
        self.cards_map = {}
        ...
        while i < self.count - 1 and self.cards_list[i] is not None:
            ...
            while j < self.count and self.cards_list[j] is not None:
                ...
                if card1 and card2:  # Handle regular set
                    ...
                    # three of a kind
                    if c1key == c2key and len(self.cards_map[c1key]) == 3:

                        print(
                            f"Yay! I found three of a kind! {c1key} {c2key} {c3key}")
                        ...
                        self.update_card_memory(
                            card1_idx, card2_idx, card3_idx)
    ...
    def checkPossibleSets(force=False):
        ...
        """
        remain_sets = int(driver.find_element_by_xpath(
            '//*[@id="setcounter"]').text[-1])
...
if __name__ == '__main__':
    ...
    TINTBS = np.array([200, 250, 200]), np.array(
        [235, 255, 235])  # Green tint boundary is 225,253,225
